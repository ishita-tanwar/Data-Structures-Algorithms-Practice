...
by completing functions heapify() and buildHeap() which are used to implement Heap Sort.
...
Your task is to complete the functions heapify(), buildheap() and heapSort() 
where heapSort() and buildheap() takes the array and it's size as input and 
heapify() takes the array, it's size and an index i as input. 
